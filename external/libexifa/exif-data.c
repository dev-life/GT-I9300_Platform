...
/* Following function added to implement the JPEG thumbnail remove feature.
   2011.06.30 - Samsung Electronics */
void
exif_data_remove_thumbnail (ExifData *data)
{
	ExifMem *mem = (data && data->priv) ? data->priv->mem : NULL;

	//Remove already obtainted IFD_1 values.
	if (data->ifd[EXIF_IFD_1]) {
		exif_content_unref (data->ifd[EXIF_IFD_1]);
		data->ifd[EXIF_IFD_1] = NULL;
	}

	//Allocate NEW IFD_1
	data->ifd[EXIF_IFD_1] = exif_content_new_mem (mem);
	if (!data->ifd[EXIF_IFD_1]) {
		exif_data_free (data);
		//ERROR!!!!!
	}
	data->ifd[EXIF_IFD_1]->parent = data;
