...
IntRect HTMLAreaElement::rect() const
{
	IntRect result ;
	if (m_shape == Rect) {
		// $NOTE- rawValue() is removed. so changed it to value()
		//	Need to check value() works same as rawValue()
		result.setX(m_coords[0].value()) ;
		result.setY(m_coords[1].value()) ;
		result.setWidth(m_coords[2].value() - result.x()) ;
		result.setHeight(m_coords[3].value() - result.y()) ;
		// -$NOTE
	}
	return result;
}
