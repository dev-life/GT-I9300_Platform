...
//HTMLComposer start
PassRefPtr<CSSValue> CSSComputedStyleDeclaration::getFontSizeCSSValueBySpecifiedSize() const
{
    if (!m_node)
        return 0;
	...
	if (!style->fontDescription().keywordSize()) {
		return primitiveValueCache->createValue(style->fontDescription().specifiedSize(), CSSPrimitiveValue::CSS_PX);
	}
	return 0;
}
//HTMLComposer end
    ...
    switch (static_cast<CSSPropertyID>(propertyID)) {
        ...
        case CSSPropertyWebkitTextSizeAdjust:
            ...
            return primitiveValueCache->createIdentifierValue(CSSValueNone);
				...
				return primitiveValueCache->createValue(style->textSizeAdjust().percentage(),  CSSPrimitiveValue::CSS_PERCENTAGE);
