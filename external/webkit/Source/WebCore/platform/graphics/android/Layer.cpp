...
// SAMSUNG CHANGE >> White flickering issue.
	m_backgroundColor = 0xffffffff; // default is white. App side should use setBackgroundColor if it wants to get rid of white flickering.
// SAMSUNG CHANGE <<
#ifdef DEBUG_TRACK_NEW_DELETE
    gLayerAllocCount += 1;
...
#if DEBUG_NAV_UI
        SkDebugf("%s index:%d  childCount:%d", __FUNCTION__,index,m_children.count());
#endif
    if ((unsigned)index < (unsigned)m_children.count()) {
        SkASSERT(m_children[index]->fParent == this);
#if DEBUG_NAV_UI
        SkDebugf("%s returning m_children[%d]", __FUNCTION__,index);
#endif
        return m_children[index];
    }
#if DEBUG_NAV_UI
        SkDebugf("%s returning NULL", __FUNCTION__);
#endif	
    return NULL;
}
