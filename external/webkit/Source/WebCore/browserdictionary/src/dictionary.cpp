...
int Dictionary::getSuggestionsTest(unsigned short *word, int length, unsigned short *outWords, int *frequencies,
        int maxWordLength, int maxWord, int maxAlternatives, int skipPos){
    mFrequencies = frequencies;
    mOutputChars = outWords;
    mInputCodesTest = word;
    mInputLength = length;
    mMaxAlternatives = maxAlternatives;
    mMaxWordLength = maxWordLength;
    mMaxWords = maxWord;
...
void
Dictionary::getWordsRecTest(int pos, int depth, int maxDepth, bool completion, int snr, int inputIndex,
                        int diffs)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (completion) {
            ...
            if (childrenAddress != 0) {
                getWordsRecTest(childrenAddress, depth + 1, maxDepth,
            ...
            mWord[depth] = c;
            if (childrenAddress != 0) {
                getWordsRecTest(childrenAddress, depth + 1, maxDepth, false, snr, inputIndex, diffs);
            }
        } else {
            int j = 0;
            while (currentChars[j] > 0) {
                if (currentChars[j] == lowerC || currentChars[j] == c) {
                    ...
                    if (mInputLength == inputIndex + 1) {
                        ...
                        if (childrenAddress != 0) {
                            getWordsRecTest(childrenAddress, depth + 1,
