...
void WMLInputElement::restoreFormControlState(const String& state)
{
    ...
    setValue(state, true); // SAMSUNG_WML_FIX
}
...
{
    setValue(String(), true); //SAMSUNG_WML_FIX
}
...
String WMLInputElement::validateInputMask(const String& inputMask)
{
    ...
    if (hasWildcard) {
        ...
        if (formatCode == '*')
            ...
            UChar formatCodeBuffer[2];
            formatCodeBuffer[0] = formatCode;
            formatCodeBuffer[1] = 0;
            unsigned leftLen = String(formatCodeBuffer).toInt();
