...
void WMLAElement::defaultEventHandler(Event* event)
{
	...
	//SAMSUNG WML CHANGES >>
    	    String parseUrl = stripLeadingAndTrailingHTMLSpaces(getAttribute(HTMLNames::hrefAttr));
            String kurl = substituteVariableReferences(parseUrl, document(), WMLVariableEscapingEscape);
	    handleLinkClick(event, document(), kurl, target(), event);
	//SAMSUNG WML CHANGES <<
            return;
        }
    }
