...
void X509Certificate::FreeOSCertHandle(OSCertHandle cert_handle) {
  ...
  // SAMSUNG CHANGE : don't free cert_handle if it has odd value +
  //                 that means it was freed somewhere else
  if ( !cert_handle || cert_handle->references < 1 || cert_handle->references > 10 ) {
  	LOG(ERROR) << "FreeOSCertHandle() invalid cert_handle(" << cert_handle->references << ")";
  	return;
  }
  // SAMSUNG CHANGE -
