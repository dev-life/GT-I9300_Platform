...
int ProxyService::ResolveProxy(const GURL& raw_url,
  ...
  if (rv != ERR_IO_PENDING){
    // SAMSUNG CHANGE: to support requesting to loopback interface, change proxy scheme to DIRECT in force.
    std::string urlStr = url.spec();
    int pos = urlStr.find("127.0.0.1");	// loopinterface address can be modified to macro value.
    if(pos != std::string::npos){
      rv = ERR_IO_PENDING;			// to change proxy scheme in DidFinishResolvingProxy(), changing error code to NOT OK value at here.
    }
    // SAMSUNG CHANGE
