...
// Praveen :: ATT read_multi_char_req (TP/GAR/CL/BI-20-C, 21)
#ifdef __PROTOCOL_TEST__
static gchar** opt_handles;
#endif
...
// KJH :: l2cap - TC_LE_CPU_BV_02_C, TC_LE_CPU_BI_01_C
#ifdef __PROTOCOL_TEST__
static gboolean opt_connect = FALSE;
#endif
...
// KJH :: gatt - TP/GAT/CL/BV-01-C, TP/GAT/CL/BV-02-C
	...
	if (opt_listen) {
		...
		g_timeout_add_seconds(30, timeout_func, NULL);
	}
#else
	if (opt_listen)
		g_idle_add(listen_start, attrib);
...
static gboolean timeout_func()
{
	g_printerr("[KJH] There is no response !!!\n");
	return FALSE;
}
#endif
...
// KJH :: GAP TP/DM/NAD/BV-01-C
	...
	if (opt_uuid != NULL) {
		...
		char_data->attrib = attribTemp;
		char_data->start = opt_start;
		char_data->end = opt_end;

		gatt_read_char_by_uuid(attribTemp, opt_start, opt_end, opt_uuid,
...
#ifdef __PROTOCOL_TEST__
	if(opt_handles != NULL){
		gatt_read_multi_char(attrib, opt_handles, char_read_cb, attrib);
		return FALSE;
	}
#endif
...
// Praveen :: ATT prepare-write-req (TP/GAW/CL/BI-11-C, 12, 17, 18)
#ifdef __PROTOCOL_TEST__
static void char_prep_write_req_cb(guint8 status, const guint8 *pdu, guint16 plen,
							gpointer user_data)
{
	g_print("Gatttool : char_prep_write_req_cb\n");
	if (status != 0) {
		g_printerr("Characteristic Prep Write Request failed: "
						"%s\n", att_ecode2str(status));
		goto done;
	}

	if (!dec_prep_write_resp(pdu, plen)) {
		g_printerr("Protocol error\n");
		goto done;
	}
...
static gboolean characteristics_prep_write_req(gpointer user_data)
{
	g_print("Gatttool : characteristics_prep_write_req\n");
	GAttrib *attrib = user_data;
	uint8_t *value;
	size_t len;
	...
	gatt_prep_write_char(attrib, opt_handle, opt_offset, value, len, char_prep_write_req_cb,
									NULL);
...
// Praveen :: ATT read_multi_char_req (TP/GAR/CL/BI-20-C, 21)
#ifdef __PROTOCOL_TEST__
	{ "handles", 0 , 0, G_OPTION_ARG_STRING_ARRAY, &opt_handles,
		"Read/Write characteristic by handles(required)", "0x0001" },
#endif
...
// KJH :: l2cap - TC_LE_CPU_BV_02_C, TC_LE_CPU_BI_01_C
#ifdef __PROTOCOL_TEST__
	{ "connect", 0, 0, G_OPTION_ARG_NONE, &opt_connect,
		"l2cap connect only", NULL },
#endif
	{ NULL },
};
