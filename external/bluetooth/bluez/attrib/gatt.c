...
guint gatt_read_multi_char(GAttrib *attrib, const char** handles, 
	...
	long_read->func = func;
	long_read->user_data = user_data;
	long_read->handle = 0x0001; //Temp : default handle

	buf = g_attrib_get_buffer(attrib, &buflen);
	plen = enc_read_mutli_char_req(handles,buf,buflen);	
	return g_attrib_send(attrib, 0, ATT_OP_READ_MULTI_REQ, buf, plen,
				read_char_helper, long_read, read_long_destroy);
}
