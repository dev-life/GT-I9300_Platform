...
// SSBT :: Praveen +
uint16_t enc_prep_write_req(uint16_t handle,  uint16_t offset,const uint8_t *value, int vlen,
							uint8_t *pdu, int len)
{
...
uint16_t dec_prep_write_resp(const uint8_t *pdu, int len)
{
	if (pdu == NULL)
		return 0;
...
uint16_t enc_read_mutli_char_req(const char **handles,uint8_t *pdu, int len)
{
	uint16_t min_len = sizeof(pdu[0]),handle;
	int i =1;
	pdu[0] = ATT_OP_READ_MULTI_REQ;	
	while(*handles != 0){
		handle = strtohandle(*handles);
		att_put_u16(handle, &pdu[i]);
		i = i+2;
		min_len = min_len + sizeof(handle);
		handles++;
	    } 
...
// SSBT :: Praveen +
	if (pdu[0] != ATT_OP_READ_RESP && pdu[0] != ATT_OP_READ_MULTI_RESP)
		return 0;
