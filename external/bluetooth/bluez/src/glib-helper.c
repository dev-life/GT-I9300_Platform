...
#define CACHE_IMS_TIMEOUT 10 /*10ms*/
...
static gboolean cached_session_expired(gpointer user_data)
{
	...
	if (device_is_authenticating(dev)) {
		/* Make sure SDP is active till authentication
		   is performed */
		return TRUE;
	}
close_session:
						...
						sdp_session_t *session)
{
	...
	if (read_remote_class(src, dst, &class) >= 0) {
		dev_type = class_to_icon(class);
		if ((NULL != dev_type) &&
			(0 == strcmp("input-mouse", dev_type))) {
			/* some mouse devices reject the incoming connection
			due to limited resources in case if SDP session is
			alive and hold the l2cap link unnecessaryly.
			reduce the sdp session cache timeout in order to address
			the above scenario*/
			cached->timer = g_timeout_add(CACHE_IMS_TIMEOUT, cached_session_expired, cached);
			return;
		}
	}
