...
int hci_le_test_end_req(int dd, int to)
{
	struct hci_request rq;
	le_test_end_rp rp;
...
int hci_le_transmitter_test(int dd, int payload_type, int ch)
{
	le_transmitter_test_cp cp;

	cp.length = 0x25;
	switch(ch)
	{
		case 0: //Low
			cp.frequency = 0x00;   //CH.0 2402MHz
			break;
		case 1: //Mid
			cp.frequency = 0x13;   //CH.19 2440MHz
			break;
		case 2: //Max
			cp.frequency = 0x27;   //CH.39 2480MHz
			break;
		case 3: //Low for inband
			cp.frequency = 0x02;   //CH.2 2406MHz
			break;
		case 4: //Max for inband
			cp.frequency = 0x25;   //CH.37 2476MHz
			break;
		default:
			break;
	}
	...
	switch(payload_type)
	{
		case 0: //PRBS9
			cp.payload = 0x00;
			break;
		case 1: //0x0F(00001111)
			cp.payload = 0x01;
			break;
		case 2: //0xAA(10101010)
			cp.payload = 0x02;
			break;
		default:
			break;
	...
	if(hci_send_cmd(dd, OGF_LE_CTL, OCF_LE_TRANSMITTER_TEST, 
	LE_TRANSMITTER_TEST_CP_SIZE, &cp) < 0)
	{
		return -1;
	}
	return 0;
...
int hci_le_receiver_test(int dd, int ch)
{
       le_receiver_test_cp cp;
       ...
       if(hci_send_cmd(dd, OGF_LE_CTL, OCF_LE_RECEIVER_TEST, 
         LE_RECEIVER_TEST_CP_SIZE, &cp) < 0)
       {
            return -1;
       }
       return 0;
...
int hci_le_test_end(int dd)
{
	if(hci_le_test_end_req(dd, 1000) <0)
	{
		return -1;
	}
	return 0;
