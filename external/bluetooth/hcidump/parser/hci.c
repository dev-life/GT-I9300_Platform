...
static inline void ext_inquiry_response_dump(int level, struct frame *frm)
{
	...
	/* eir data parsing error fix. */
	/* length = get_u8(frm); */
	length = *((uint8_t *) frm->ptr);
...
static inline void le_read_rx_test_end_response_dump(int level, struct frame *frm)
{
	le_test_end_rp *rp = frm->ptr;

	p_indent(level, frm);
	printf("status 0x%2.2x num_pkts 0x%2.2x(%d)\n", rp->status,	rp->num_pkts, rp->num_pkts);
	if (rp->status > 0) {
		p_indent(level, frm);
		printf("Error: %s\n", status2str(rp->status));
	}
...
static inline void cmd_complete_dump(int level, struct frame *frm)
{
	...
	case OGF_LE_CTL:
		...
		case OCF_LE_TRANSMITTER_TEST:
			status_response_dump(level, frm);
			return;
		case OCF_LE_TEST_END:
			le_read_rx_test_end_response_dump(level, frm);
			printf("OCF_LE_TEST_END\n");
			return;
		case OCF_LE_READ_BUFFER_SIZE:
